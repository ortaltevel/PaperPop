...
module.exports=async function(req,res){
 ...
 try{const{getOrder,ensureEmailDeliveryColumns}=require("./_lib/db");await ensureEmailDeliveryColumns();const order=await getOrder(id);if(!order)return res.status(404).json({error:"ORDER_NOT_FOUND"});return res.status(200).json({status:order.status,orderNumber:String(order.order_number)})}catch(e){console.error("order_status_failed",{requestId:require("node:crypto").randomUUID(),code:e.message});return res.status(500).json({error:"STATUS_UNAVAILABLE"})}
};
