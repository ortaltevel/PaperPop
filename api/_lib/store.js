...
const CATALOG = Object.freeze({ octopus:{name:"התמנון שעושה סדר",price:45}, duck:{name:"הברווז השובב",price:65}, heart:{name:"הלב הפועם",price:45}, soccer:{name:"הכדור שלא מפספס",price:45} });
...
function validate(input){
 if(!input||!Array.isArray(input.items)||!input.items.length||input.items.length>4)throw Error("INVALID_CART");
 const items=input.items.map(r=>{const p=CATALOG[r.id],q=Number(r.quantity);if(!p||!Number.isInteger(q)||q<1||q>99)throw Error("INVALID_CART");return{id:r.id,name:p.name,unitPrice:p.price,quantity:q,total:p.price*q}});
